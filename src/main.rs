...
impl State for GameState {
    ...
    fn update(&mut self, ctx: &mut Context) -> tetra::Result {
        ...
        let player1_bounds = self.player1.get_bounds();
        let player2_bounds = self.player2.get_bounds();
        let ball_bounds = self.ball.get_bounds();
        ...
        if let Some(paddle) = paddle_hit {
            ...
            let offset = (paddle.get_centre() - self.ball.get_centre()) / paddle.texture.height() as f32;
            self.ball.velocity.y += offset * SPIN_CONSTANT * offset.signum();
...
impl Player {
    ...
    fn get_centre(&self) -> f32 {
        (self.texture.height() as f32 / 2.0) + self.params.position.y
    }
}
...
impl Ball {
    ...
    fn get_centre(&self) -> f32 {
        (self.texture.height() as f32 / 2.0) + self.position.y
    }
}
